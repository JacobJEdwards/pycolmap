...
class SceneManager:
    ...
    def _save_images_txt(self, output_file):
        with open(output_file, "w") as fid:
            ...
            for image_id, image in self.images.iteritems():
                ...
                (
                    print >> fid,
                    " ".join(str(qi) for qi in image.q.q),
                )
                (
                    print >> fid,
                    " ".join(str(ti) for ti in image.tvec),
                )
    ...
    def _save_points3D_txt(self, output_file):
        ...
        with open(output_file, "w") as fid:
            ...
            (
                print >> fid,
                "# Number of points: {},".format(num_valid_points3D),
            )
            ...
            for point3D_id, point3D_idx in iter_point3D_id_to_point3D_idx:
                ...
                (
                    print >> fid,
                    array_to_string(self.points3D[point3D_idx]),
                )
                (
                    print >> fid,
                    array_to_string(self.point3D_colors[point3D_idx]),
                )
                (
                    print >> fid,
